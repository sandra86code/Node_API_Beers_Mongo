...
async function addBar(req = request, res = response){
    ...
    }else if(bar.Nombre.trim().length<1 || bar.Nombre.trim().length>25) {
        res.json({ message: 'El nombre del bar debe estar entre 1 y 25 caracteres.' });
...
async function editBar(req = request, res = response){
    ...
    if (updatedBar.length) {
        ...
        res.json(await User.find({ _id: barId }));
    } else {
        res.json({ message: `El bar con id ${barId} no existe` })
    }
}
